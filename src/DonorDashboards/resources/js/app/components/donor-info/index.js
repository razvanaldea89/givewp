...
const DonorInfo = () => {
    const {name, addresses, company, sinceLastDonation, sinceCreated, avatarUrl, initials} = useSelector(
        (state) => state.profile
    );
            ...
            <div className="give-donor-dashboard-donor-info__details">
                ...
                {address && (
                    <div className="give-donor-dashboard-donor-info__detail">
                        <FontAwesomeIcon icon="map-pin" fixedWidth={true} /> {address.city},{' '}
                        {address.state.length > 0 ? address.state : address.country}
                    </div>
                )}
                {company && (
                    <div className="give-donor-dashboard-donor-info__detail">
                ...
                {sinceLastDonation && (
                    <div className="give-donor-dashboard-donor-info__detail">
                        <FontAwesomeIcon icon="clock" fixedWidth={true} />{' '}
                        {sprintf(__('Last donated %s ago', 'give'), sinceLastDonation)}
                    </div>
                )}
                {sinceCreated && (
                    <div className="give-donor-dashboard-donor-info__detail">
                        <FontAwesomeIcon icon="heart" fixedWidth={true} />{' '}
                        {sprintf(__('Donor for %s', 'give'), sinceCreated)}
                    </div>
                )}
            </div>
