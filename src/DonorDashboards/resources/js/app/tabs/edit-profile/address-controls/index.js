...
const AddressControls = ({
    ...
    const additionalAddressControls = additionalAddresses
        ? additionalAddresses.map((address, index) => {
              return (
                  <Fragment key={index}>
                      <FieldRow>
                          <Heading>{__('Additional Address', 'give')}</Heading>
                          <div className="give-donor-dashboard__address-controls">
                              <div
                                  className="give-donor-dashboard__make-primary-address"
                                  onClick={() => makePrimaryAddress(address, index)}
                              >
                                  {__('Make Primary', 'give')}
                              </div>
                              |
                              <div
                                  className="give-donor-dashboard__delete-address"
                      ...
                      <Divider />
                      <AddressFields address={address} onChange={(value) => setAdditionalAddress(value, index)} />
                  </Fragment>
              );
          })
        : null;
    ...
    return primaryAddress ? (
        <Fragment>
            <Heading>{__('Primary Address', 'give')}</Heading>
            <Divider />
        ...
        <div className="give-donor-dashboard__add-primary-address">
            <Heading>{__('Looks like you have not set up an address!', 'give')}</Heading>
